...
// ParrotMod returns parrot mod.
func ParrotMod() string {
	...
	if path, err := ModuleVersion("github.com/go-parrot/parrot/v2"); err == nil {
		// $GOPATH/pkg/mod/github.com/go-parrot/parrot@v2
		return filepath.Join(cachePath, path)
	}
	// $GOPATH/src/github.com/go-parrot/parrot
	return filepath.Join(gopath, "src", "github.com", "go-parrot", "parrot")
}
