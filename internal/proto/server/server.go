...
var CmdServer = &cobra.Command{
	...
	Long:  "Generate the proto Server implementations. eg: parrot proto server api/xxx.proto -target-dir=internal/logic",
	Run:   run,
}
var targetDir string

func init() {
	CmdServer.Flags().StringVarP(&targetDir, "target-dir", "t", "internal/logic", "generate target directory")
}

func run(cmd *cobra.Command, args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "Please specify the proto file. eg: parrot proto server api/xxx.proto")
		return
	}
